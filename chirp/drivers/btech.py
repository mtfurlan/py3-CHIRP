...
def _recv(radio, addr):
    ...
    if a != addr or l != BLOCK_SIZE or c != ord("X"):
        print("Invalid header for block 0x%04x" % addr)
        print("CMD: %s  ADDR: %04x  SIZE: %02x" % (c, a, l))
...
def _do_ident(radio, status, upload=False):
    ...
    if itis is False:
        print("Incorrect model ID, got this:\n\n" + util.hexprint(ident))
...
class BTechMobileCommon(chirp_common.CloneModeRadio,
    ...
    def set_memory(self, mem):
        ...
        if _mem.get_raw()[0] == "\xFF":
            print("This mem was empty before")
            mem_was_empty = True
        ...
        if len(mem.extra) > 0:
            ...
            print("Extra-Setting supplied. Setting them.")
            ...
            if mem.empty:
                print("New mem is empty.")
            else:
                print("New mem is NOT empty")
                # set extra-settings to default ONLY when apreviously empty or
                # deleted memory was edited to prevent errors such as #4121
                if mem_was_empty:
                    print("old mem was empty. Setting default for extras.")
                    _zero_settings()
    ...
    def get_settings(self):
        ...
        def apply_dmtf_frame(setting, obj):
            print("Setting DTMF-Code: " + str(setting.value))
        ...
        def apply_dmtf_listvalue(setting, obj):
            print("Setting value: " + str(setting.value) + " from list")
        ...
        for standard in _5tone_standards:
            ...
            if period == 255:
                print("Period for " + LIST_5TONE_STANDARDS[i] +
            ...
            if period <= len(LIST_5TONE_STANDARD_PERIODS):
                ...
                print("Invalid value for 5tone period! Disabling.")

            group_tone = standard.group_tone
            if group_tone == 255:
                print("Group-Tone for " + LIST_5TONE_STANDARDS[i] +
            ...
            if group_tone <= len(LIST_5TONE_DIGITS):
                ...
                print("Invalid value for 5tone digit! Disabling.")

            repeat_tone = standard.repeat_tone
            if repeat_tone == 255:
                print("Repeat-Tone for " + LIST_5TONE_STANDARDS[i] +
        ...
        def apply_5tone_frame(setting, obj):
            print("Setting 5 Tone: " + str(setting.value))
            valstring = str(setting.value)
            if len(valstring) == 0:
                for i in range(0, 5):
                    obj[i] = 255
                ...
                for i in range(0, 5):
                    ...
                    if currentChar in LIST_5TONE_DIGITS:
                        ...
                        validFrame = False
                        print("invalid char: " + str(currentChar))
                if not validFrame:
                    print("setting whole frame to FF")
                    for i in range(0, 5):
                        obj[i] = 255
        ...
        if _5tone_delay2 == 255:
            _5tone_delay2 = 20
            print("5 Tone delay unconfigured! Resetting to 200ms.")
        ...
        if _5tone_delay2 <= len(LIST_5TONE_DELAY):
            ...
            print("Invalid value for 5tone delay (frame2)! Disabling.")
        ...
        if _5tone_delay3 <= len(LIST_5TONE_DELAY):
            ...
            print("Invalid value for 5tone delay (frame3)! Disabling.")

        ext_length = _mem._5tone_settings._5tone_first_digit_ext_length
        if ext_length == 255:
            ext_length = 0
            print("1st Tone ext length unconfigured! Resetting to 0")
        ...
        if decode_reset_time <= len(LIST_5TONE_RESET):
            ...
            print("Invalid value decode reset time! Disabling.")
        ...
        if duration_1st_tone == 255:
            print("Duration of first 2 Tone digit is not yet " +
        ...
        if duration_2nd_tone == 255:
            print("Duration of second 2 Tone digit is not yet " +
                      "configured. Setting to 600ms")
        ...
        if duration_gap == 255:
            print("Duration of gap is not yet " +
        ...
        for decode_code in self._memobj._2tone._2tone_decode:
            ...
            for dec in decode_code.decs:
                val = dec.dec
                if val == 255:
                    print("Dec for Code " + str(i) + " Dec " + str(j) +
                ...
                if val <= len(LIST_2TONE_DEC):
                    ...
                    print("Invalid value for 2tone dec! Disabling.")

                val = dec.response
                if val == 255:
                    print("Response for Code " +
                              str(i) + " Dec " + str(j) +
                              " is not yet configured. Setting to 0.")
                    val = 0
                ...
                if val <= len(PTTIDCODE_LIST):
                    ...
                    print("Invalid value for 2tone alert! Disabling.")
                j = j + 1
